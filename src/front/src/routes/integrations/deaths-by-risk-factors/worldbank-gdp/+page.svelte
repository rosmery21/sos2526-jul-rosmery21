...
/* eslint-disable no-undef */
    ...
    async function loadCountries() {
        try {
            const res = await fetch("/api/v2/deaths-by-risk-factors/integrations/worldbank-gdp/countries");
            if (!res.ok) return;
    ...
    async function loadGDP() {
        ...
        errorMsg = ""; 
        try {
            const res = await fetch(`/api/v2/deaths-by-risk-factors/integrations/worldbank-gdp?country=${encodeURIComponent(selectedCountry)}`);
            if (res.ok) {
...
<svelte:head>
    ...
    <!-- CORRECCIÓN: Usar onload en lugar de on:load para Svelte 5 -->
...
<main class="container mt-4 mb-5">
    ...
    <div class="row justify-content-center mb-4">
        <div class="col-md-8 card shadow-sm p-4 bg-light">
            ...
            <div class="input-group">
                <select id="countrySelect" class="form-select" bind:value={selectedCountry} onchange={loadGDP}>
    ...
    <div id="chart-variwide" class="border rounded shadow-sm bg-white" style="width:90%; height:550px;"></div>

    <p>En esta gráfica podemos ver la evolucion del PIB del país seleccionado durante los últimos años. Cuanto
        más ancha sea la columna del año, mayor será el volumen del PIB del país indicado en ese año.
    </p>
